...
bool cot_weight = false;
bool rotate = false;
float myRotation[4] = { 0, 0, 0, 1 }; //BONUS: interactively specify the rotation for the Laplacian coordinates at the handles
...
MatX3f meshX;
MatX3i meshF; // index start from 0

void deform_preprocess()
{

    std::vector<bool> uniform = {uniform_weight};
    eigen2matlab("x", meshX.cast<double>());
    eigen2matlab("f", meshF.cast<int>());
    matlabEval("f = double(f+1);"); 
    vector2matlab("uniform", uniform);
    matlabEval("deform;"); // run deform file
...
void meshDeform()
{
    ...
    std::vector<int> P2PVtxIds = M.getConstrainVertexIds(); // can get constrain vex num here
    ...
    if (rotate == true) {
        std::vector<double> Myrot = { myRotation[0],myRotation[1],myRotation[2],myRotation[3] };
        vector2matlab("rot", Myrot);              
    }
    else {
        std::vector<double> Myrot = { 0,0,0,1 };
        vector2matlab("rot", Myrot);
    }
...
void createTweakbar()
{
    ...
    TwBar* Editbar = TwGetBarByName("MeshEditing");
    if (Editbar)    TwDeleteBar(Editbar);
    //Create a tweak bar
    ...
    Editbar = TwNewBar("MeshEditing");
    TwDefine(" MeshEditing size='280 220' color='114 114 114' text=dark alpha=128 position='125 5'");
    ...
    TwAddButton(Editbar, "Uniform Weight", [](void*) {
        uniform_weight = true;
        cot_weight = false;
        deform_preprocess();
        meshDeform();
        ...
        &uniform_weight, "");
    TwAddVarRW(Editbar, "Uniform Weight result", TW_TYPE_BOOLCPP, &uniform_weight, "");
    TwAddButton(Editbar, "Cot Weight", [](void*) {
        ...
        &cot_weight, "");
    TwAddVarRW(Editbar, "Cot Weight result", TW_TYPE_BOOLCPP, &cot_weight, "");
    TwAddVarRW(Editbar, "Rotation", TW_TYPE_QUAT4F, myRotation, "");
    TwAddVarRW(Editbar, "Rotate result", TW_TYPE_BOOLCPP, &rotate, "");
    TwAddButton(Editbar, "rotate", [](void*) {
        rotate = true;
        deform_preprocess();
        meshDeform();
        glutPostRedisplay();
        ...
        &rotate, "");
    TwAddButton(Editbar, "ClearRotate", [](void*) {
        rotate = false;
        deform_preprocess();
        meshDeform();
        glutPostRedisplay();
